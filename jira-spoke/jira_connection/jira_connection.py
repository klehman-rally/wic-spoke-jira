...
import wicore
from wicore import WICConnection
...
require 'rallyeif/jira/utils/jira_xsd'
    ...
    class JiraConnection(WICConnection):

      #include JiraWorkflow  # this should be a mixin for this class

      MAX_NUM_RESULTS = 9999
      JIRA_CLOSED_STATUS = 'Closed'

      #JIRA permissions required to run the connector
      on_prem_required_permissions = ['BROWSE', 'CREATE_ISSUE', 'EDIT_ISSUE', 'SCHEDULE_ISSUE', 
                                      'ASSIGN_ISSUE', 'ASSIGNABLE_USER', 
                                      'RESOLVE_ISSUE', 'CLOSE_ISSUE', 'MODIFY_REPORTER',
                                      'COMMENT_ISSUE', 'CREATE_ATTACHMENT'
                                     ]
      on_demand_required_permissions = [
                           'BROWSE_PROJECTS', 'CREATE_ISSUES', 'EDIT_ISSUES', 'SCHEDULE_ISSUES', 
                           'ASSIGN_ISSUES', 'ASSIGNABLE_USER', 
                           'RESOLVE_ISSUES', 'CLOSE_ISSUES', 'MODIFY_REPORTER',
      ...
      def __init__(self, config=None):
          super.__init__(self)
          self.jira_timeout = None
          config and self.readConfig(config)
          self.jira_version = "unknown version"
      ...
      def version(self):
          return "----  RallyEIF::WRK::Jira::Version =----"

      def get_backend_version(self):
          return f'{self.name()} {self.jira_version}'

      def readConfig(self, config):
          super.__init__(self, config)
          self.url        = config.get("Url", None)
          self.account_id = config.get("AccountID", None)
          self.project    = config.get("Project", None)
          self.id_field   = 'key' if self.id_field is None else self.id_field
          self.workflow_file = config.get("WorkflowFile", None) or 'jira_workflow.xml'

          self.workflow = RallyEIF::WRK::Utils::JiraWorkflowReader.read_workflow_file(Dir.pwd + '/' + @workflow_file)
          if self.workflow is None:
              raise ConfigurationError(f'Could not read workflow from {self.workflow_file}')

        # de-symbolize the identity related field names,
        # jirpa prefers field name references be a JIRA DisplayName String, so we may as well get that done here...
        ...
        if @jira_open_timeout
          if @jira_open_timeout !~ /^\d+$/
            ...
            raise raise ConfigurationError(problem, self)
          end
        end
        ...
        if @jira_open_timeout and @jira_open_timeout > 0 and @jira_open_timeout > 999
          ...
          raise ConfigurationError(problem, self)
        end
      ...
      def connect()
        ...
        begin
          ...
          raise ConfigurationError("Attempt to open connection to JIRA timed out", self)
        rescue Exception => ex
        ...
        if not appropriate_jira_version(@jira_version, :min_major => 5, :min_minor => 2)
          raise ConfigurationError("JIRA version needs to be 5.2.x or better, current version is #{@jira_version}", self)
        end
        ...
        if not system_error.empty?
          ...
          raise ConfigurationError(problem_description + system_error.join(", "), self)
        end
        if not config_error.empty?
          problem_description = "JIRA Permissions incorrect for "
          raise ConfigurationError(problem_description + config_error.join(", "), self)
        end
        ...
        if not jira_issue_type
          ...
          raise ConfigurationError(unrecognized_type, self)
        end
      ...
      def validate_project()
        if not @project or @project.nil? or @project.empty?
          raise ConfigurationError("A valid Jira project must be specified", self)
        end
...
##
      ...
      def checkResultAgainstIntent(intent, issue)
        ...
        for field_name in intent.keys do
          ...
          if actual_value.to_s != intent[field_name].to_s
            ...
            #In the case of Description empty came back as None where intent was "", but essentially they are equivalent
      ...
      def update_internal(issue, int_work_item)
        ...
        desired_status = status_key ? int_work_item[status_key] : None
      ...
      def attempt_update(issue, int_work_item)
        ...
        begin
          ...
          raise ConfigurationError("Unable to update issue #{issue.key}, #{ex.message}", self)
        end
      ...
      def update_status(issue_key, issue, int_work_item, desired_status, current_status=None)
##
...
##
      ...
      def update_external_id_fields(issue, external_id, end_user_id=None, item_link=None)
      ...
      def normalize_link(link)
        link =~ /<a href="(.*)">.*<\/a>/i
        $1 || link # returns url if None returns original link
      end
      ...
      def find_new()
        ...
        issues = []
        begin
          issues = @jira_proxy.getIssuesWithJql(complete_query, None, 0, MAX_NUM_RESULTS)
        rescue Exception => ex
      ...
      def find_by_external_id(external_id)
        ...
        begin
          issues = @jira_proxy.getIssuesWithJql(query, None, 0, MAX_NUM_RESULTS)
        rescue Exception => ex
